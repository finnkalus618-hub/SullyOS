import { isOmbreMemorySelected, shouldReadLocalMemory } from './memoryBackend';
import { recallOmbreMemory } from './ombreMemory';
...
export async function buildChatRequestPayload(input: BuildChatPayloadInput): Promise<BuildChatPayloadResult> {
    ...
    // ── 1. 统一记忆总线路由 (OB 优先 / 本地回退) ───────────
    let ombreMemoryBlock = '';
    if (isOmbreMemorySelected()) {
        const queryHint = input.recallQueryHint || (recentMsgsHint && recentMsgsHint.length > 0 ? recentMsgsHint[recentMsgsHint.length - 1].content : '');
        const obRes = await recallOmbreMemory({
            charId: char.id,
            charName: char.name,
            userName: userProfile?.name || '林小姐',
            query: queryHint
        });
        if (obRes.content) {
            ombreMemoryBlock = `
...
${obRes.content}`;
    ...
    // 只有在允许读取本地记忆时才执行原生 Memory Palace 召回
    const allowLocal = shouldReadLocalMemory(!ombreMemoryBlock);
    const recallTrace = allowLocal
    ...
    let systemPrompt = parts.stable + (ombreMemoryBlock || '');
